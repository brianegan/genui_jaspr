// Generated path data is intentionally kept verbatim.
...
const _paths = {
  ...
  'arrowBack': 'M20 11H7.83l5.59-5.59L12 4l-8 8l8 8l1.41-1.41L7.83 13H20v-2z',
  ...
  'rewind': 'M11 18V6l-8.5 6l8.5 6zm.5-6l8.5 6V6l-8.5 6z',
  'search':
  ...
  'volumeMute': 'M7 9v6h4l5 5V4l-5 5H7z',
  'volumeOff':
  ...
  'warning': 'M1 21h22L12 2L1 21zm12-3h-2v-2h2v2zm0-4h-2v-4h2v4z',
};

/// Builds an icon for a resolved A2UI icon name.
typedef IconRenderer = Component Function(String name);
...
abstract final class IconComponent extends JasprComponent {
  ...
  /// Creates an [IconComponent] backed only by [renderer].
  ///
...
final class _StandardIconComponent extends IconComponent {
  ...
  Component build(ComponentScope scope) {
    final name = scope.string('name');
    final iconPath = _paths[name];
    if (iconPath == null) {
      scope.reportError(
        ArgumentError.value(name, 'name', 'Unknown standard icon'),
      );
...
final class _CustomIconComponent extends IconComponent {
  ...
  Component build(ComponentScope scope) {
    final name = scope.string('name');
    if (name == null) {
      scope.reportError(ArgumentError.notNull('name'));
      return const Component.empty();
    }
